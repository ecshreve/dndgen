...
CREATE TABLE `new_proficiencies` (`id` integer NOT NULL PRIMARY KEY AUTOINCREMENT, `indx` text NOT NULL, `name` text NOT NULL, `proficiency_category` text NOT NULL, `choice_starting_proficiency_options` integer NULL, `proficiency_skill` integer NULL, `proficiency_equipment` integer NULL, `proficiency_saving_throw` integer NULL, CONSTRAINT `proficiencies_choices_starting_proficiency_options` FOREIGN KEY (`choice_starting_proficiency_options`) REFERENCES `choices` (`id`) ON DELETE SET NULL, CONSTRAINT `proficiencies_skills_skill` FOREIGN KEY (`proficiency_skill`) REFERENCES `skills` (`id`) ON DELETE SET NULL, CONSTRAINT `proficiencies_equipment_equipment` FOREIGN KEY (`proficiency_equipment`) REFERENCES `equipment` (`id`) ON DELETE SET NULL, CONSTRAINT `proficiencies_ability_scores_saving_throw` FOREIGN KEY (`proficiency_saving_throw`) REFERENCES `ability_scores` (`id`) ON DELETE SET NULL);
...
CREATE TABLE `new_races` (`id` integer NOT NULL PRIMARY KEY AUTOINCREMENT, `indx` text NOT NULL, `name` text NOT NULL, `alignment` text NOT NULL, `age` text NOT NULL, `size` text NOT NULL, `size_description` text NOT NULL, `language_desc` text NOT NULL, `speed` integer NOT NULL, `choice_from` integer NULL, `race_starting_proficiency_options` integer NULL, CONSTRAINT `races_choices_from` FOREIGN KEY (`choice_from`) REFERENCES `choices` (`id`) ON DELETE SET NULL, CONSTRAINT `races_choices_starting_proficiency_options` FOREIGN KEY (`race_starting_proficiency_options`) REFERENCES `choices` (`id`) ON DELETE SET NULL);
...
INSERT INTO `new_races` (`id`, `indx`, `name`, `alignment`, `age`, `size`, `size_description`, `language_desc`, `speed`, `race_starting_proficiency_options`) SELECT `id`, `indx`, `name`, `alignment`, `age`, `size`, `size_description`, `language_desc`, `speed`, `race_starting_proficiency_options` FROM `races`;
