...
func (rq *RaceQuery) loadLanguages(ctx context.Context, query *LanguageQuery, nodes []*Race, init func(*Race), assign func(*Race, *Language)) error {
	...
	query.Where(predicate.Language(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(race.LanguagesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.race_languages
		if fk == nil {
			return fmt.Errorf(`foreign-key "race_languages" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "race_languages" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
