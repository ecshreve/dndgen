...
func (pcc *ProficiencyChoiceCreate) AddOptionIDs(ids ...int) *ProficiencyChoiceCreate {
	pcc.mutation.AddOptionIDs(ids...)
	return pcc
}

// AddOptions adds the "options" edges to the Proficiency entity.
func (pcc *ProficiencyChoiceCreate) AddOptions(p ...*Proficiency) *ProficiencyChoiceCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return pcc.AddOptionIDs(ids...)
}
...
func (pcc *ProficiencyChoiceCreate) createSpec() (*ProficiencyChoice, *sqlgraph.CreateSpec) {
	...
	if nodes := pcc.mutation.OptionsIDs(); len(nodes) > 0 {
