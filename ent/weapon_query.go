...
func (wq *WeaponQuery) loadRange(ctx context.Context, query *WeaponRangeQuery, nodes []*Weapon, init func(*Weapon), assign func(*Weapon, *WeaponRange)) error {
	...
	query.withFKs = true
	query.Where(predicate.WeaponRange(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(weapon.RangeColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.weapon_range
		if fk == nil {
			return fmt.Errorf(`foreign-key "weapon_range" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "weapon_range" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (wq *WeaponQuery) loadDamage(ctx context.Context, query *WeaponDamageQuery, nodes []*Weapon, init func(*Weapon), assign func(*Weapon, *WeaponDamage)) error {
	...
	query.Where(predicate.WeaponDamage(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(weapon.DamageColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.weapon_damage
		if fk == nil {
			return fmt.Errorf(`foreign-key "weapon_damage" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "weapon_damage" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
