...
func (lq *LanguageQuery) QueryRaces() *RaceQuery {
...
func (lq *LanguageQuery) WithRaces(opts ...func(*RaceQuery)) *LanguageQuery {
	query := (&RaceClient{config: lq.config}).Query()
...
func (lq *LanguageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Language, error) {
	...
	if query := lq.withRaces; query != nil {
		if err := lq.loadRaces(ctx, query, nodes,
			func(n *Language) { n.Edges.Races = []*Race{} },
			func(n *Language, e *Race) { n.Edges.Races = append(n.Edges.Races, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range lq.withNamedRaces {
		if err := lq.loadRaces(ctx, query, nodes,
			func(n *Language) { n.appendNamedRaces(name) },
			func(n *Language, e *Race) { n.appendNamedRaces(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (lq *LanguageQuery) loadRaces(ctx context.Context, query *RaceQuery, nodes []*Language, init func(*Language), assign func(*Language, *Race)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(language.RacesTable)
		s.Join(joinT).On(s.C(race.FieldID), joinT.C(language.RacesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(language.RacesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(language.RacesPrimaryKey[1]))
...
func (lq *LanguageQuery) WithNamedRaces(name string, opts ...func(*RaceQuery)) *LanguageQuery {
	query := (&RaceClient{config: lq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if lq.withNamedRaces == nil {
		lq.withNamedRaces = make(map[string]*RaceQuery)
	}
	lq.withNamedRaces[name] = query
	return lq
}
