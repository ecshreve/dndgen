...
func (ecc *EquipmentCategoryCreate) SetIndx(e equipmentcategory.Indx) *EquipmentCategoryCreate {
	ecc.mutation.SetIndx(e)
	return ecc
}

// SetNillableIndx sets the "indx" field if the given value is not nil.
func (ecc *EquipmentCategoryCreate) SetNillableIndx(e *equipmentcategory.Indx) *EquipmentCategoryCreate {
	if e != nil {
		ecc.SetIndx(*e)
	}
	return ecc
...
func (ecc *EquipmentCategoryCreate) defaults() {
	if _, ok := ecc.mutation.Indx(); !ok {
		v := equipmentcategory.DefaultIndx
		ecc.mutation.SetIndx(v)
	}
}
