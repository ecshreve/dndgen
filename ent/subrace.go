...
type SubraceEdges struct {
	...
	namedLanguageOptions map[string][]*LanguageChoice
}
...
// was not loaded in eager-loading.
func (e SubraceEdges) LanguageOptionsOrErr() ([]*LanguageChoice, error) {
	if e.loadedTypes[4] {
...
func (*Subrace) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case subrace.FieldIndx, subrace.FieldName:
...
// QueryLanguageOptions queries the "language_options" edge of the Subrace entity.
func (s *Subrace) QueryLanguageOptions() *LanguageChoiceQuery {
	return NewSubraceClient(s.config).QueryLanguageOptions(s)
}
...
// NamedLanguageOptions returns the LanguageOptions named value or an error if the edge was not
// loaded in eager-loading with this name.
func (s *Subrace) NamedLanguageOptions(name string) ([]*LanguageChoice, error) {
	if s.Edges.namedLanguageOptions == nil {
		return nil, &NotLoadedError{edge: name}
	}
	nodes, ok := s.Edges.namedLanguageOptions[name]
	if !ok {
		return nil, &NotLoadedError{edge: name}
	}
	return nodes, nil
...
func (s *Subrace) appendNamedLanguageOptions(name string, edges ...*LanguageChoice) {
	if s.Edges.namedLanguageOptions == nil {
		s.Edges.namedLanguageOptions = make(map[string][]*LanguageChoice)
	}
	if len(edges) == 0 {
		s.Edges.namedLanguageOptions[name] = []*LanguageChoice{}
	} else {
		s.Edges.namedLanguageOptions[name] = append(s.Edges.namedLanguageOptions[name], edges...)
	}
}
