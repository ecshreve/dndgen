...
func (cq *CharacterQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Character, error) {
	...
	if query := cq.withRace; query != nil {
		if err := cq.loadRace(ctx, query, nodes,
			func(n *Character) { n.Edges.Race = []*Race{} },
			func(n *Character, e *Race) { n.Edges.Race = append(n.Edges.Race, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withClass; query != nil {
		if err := cq.loadClass(ctx, query, nodes,
			func(n *Character) { n.Edges.Class = []*Class{} },
			func(n *Character, e *Class) { n.Edges.Class = append(n.Edges.Class, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range cq.withNamedRace {
		if err := cq.loadRace(ctx, query, nodes,
			func(n *Character) { n.appendNamedRace(name) },
			func(n *Character, e *Race) { n.appendNamedRace(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range cq.withNamedClass {
		if err := cq.loadClass(ctx, query, nodes,
			func(n *Character) { n.appendNamedClass(name) },
			func(n *Character, e *Class) { n.appendNamedClass(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (cq *CharacterQuery) loadRace(ctx context.Context, query *RaceQuery, nodes []*Character, init func(*Character), assign func(*Character, *Race)) error {
	...
	query.Where(predicate.Race(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(character.RaceColumn), fks...))
	}))
...
func (cq *CharacterQuery) loadClass(ctx context.Context, query *ClassQuery, nodes []*Character, init func(*Character), assign func(*Character, *Class)) error {
	...
	query.Where(predicate.Class(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(character.ClassColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (cq *CharacterQuery) WithNamedRace(name string, opts ...func(*RaceQuery)) *CharacterQuery {
	query := (&RaceClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (cq *CharacterQuery) WithNamedClass(name string, opts ...func(*ClassQuery)) *CharacterQuery {
	query := (&ClassClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
