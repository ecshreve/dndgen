...
func (pq *PropertyQuery) QueryWeapons() *WeaponQuery {
	query := (&WeaponClient{config: pq.config}).Query()
...
func (pq *PropertyQuery) Clone() *PropertyQuery {
	...
	return &PropertyQuery{
		...
		withWeapons: pq.withWeapons.Clone(),
...
func (pq *PropertyQuery) WithWeapons(opts ...func(*WeaponQuery)) *PropertyQuery {
	query := (&WeaponClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withWeapons = query
	return pq
}
...
func (pq *PropertyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Property, error) {
	...
	if query := pq.withWeapons; query != nil {
		if err := pq.loadWeapons(ctx, query, nodes,
			func(n *Property) { n.Edges.Weapons = []*Weapon{} },
			func(n *Property, e *Weapon) { n.Edges.Weapons = append(n.Edges.Weapons, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range pq.withNamedWeapons {
		if err := pq.loadWeapons(ctx, query, nodes,
			func(n *Property) { n.appendNamedWeapons(name) },
			func(n *Property, e *Weapon) { n.appendNamedWeapons(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (pq *PropertyQuery) loadWeapons(ctx context.Context, query *WeaponQuery, nodes []*Property, init func(*Property), assign func(*Property, *Weapon)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(property.WeaponsTable)
		s.Join(joinT).On(s.C(weapon.FieldID), joinT.C(property.WeaponsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(property.WeaponsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(property.WeaponsPrimaryKey[1]))
...
func (pq *PropertyQuery) WithNamedWeapons(name string, opts ...func(*WeaponQuery)) *PropertyQuery {
	query := (&WeaponClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if pq.withNamedWeapons == nil {
		pq.withNamedWeapons = make(map[string]*WeaponQuery)
	}
	pq.withNamedWeapons[name] = query
	return pq
}
