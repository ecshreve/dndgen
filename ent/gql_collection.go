...
func (c *ChoiceQuery) collectField(ctx context.Context, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "proficiencyOptions":
			...
			c.WithNamedProficiencyOptions(alias, func(wq *ProficiencyQuery) {
				*wq = *query
			})
		case "class":
			var (
		...
		case "desc":
			if _, ok := fieldSeen[choice.FieldDesc]; !ok {
				selectedFields = append(selectedFields, choice.FieldDesc)
				fieldSeen[choice.FieldDesc] = struct{}{}
			}
		case "id":
		case "__typename":
		default:
...
func (c *ClassQuery) collectField(ctx context.Context, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "proficiencyChoices":
			...
			c.WithNamedProficiencyChoices(alias, func(wq *ChoiceQuery) {
				*wq = *query
			})
