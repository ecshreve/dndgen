...
func (eq *EquipmentQuery) loadEquipmentCategory(ctx context.Context, query *EquipmentCategoryQuery, nodes []*Equipment, init func(*Equipment), assign func(*Equipment, *EquipmentCategory)) error {
	...
	for i := range nodes {
		if nodes[i].equipment_category_equipment == nil {
			continue
		}
		fk := *nodes[i].equipment_category_equipment
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "equipment_category_equipment" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
