...
func (sc *SubraceCreate) AddLanguageOptionIDs(ids ...int) *SubraceCreate {
	sc.mutation.AddLanguageOptionIDs(ids...)
	return sc
}

// AddLanguageOptions adds the "language_options" edges to the LanguageChoice entity.
func (sc *SubraceCreate) AddLanguageOptions(l ...*LanguageChoice) *SubraceCreate {
	ids := make([]int, len(l))
	for i := range l {
		ids[i] = l[i].ID
	}
	return sc.AddLanguageOptionIDs(ids...)
}
...
func (sc *SubraceCreate) createSpec() (*Subrace, *sqlgraph.CreateSpec) {
	...
	if nodes := sc.mutation.LanguageOptionsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
