...
func (e PrerequisiteEdges) AbilityScoreOrErr() ([]*AbilityScore, error) {
	if e.loadedTypes[0] {
...
func (*Prerequisite) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case prerequisite.FieldID, prerequisite.FieldMinimum:
...
// QueryAbilityScore queries the "ability_score" edge of the Prerequisite entity.
func (pr *Prerequisite) QueryAbilityScore() *AbilityScoreQuery {
	return NewPrerequisiteClient(pr.config).QueryAbilityScore(pr)
}
