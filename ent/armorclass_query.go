...
func (acq *ArmorClassQuery) QueryArmor() *ArmorQuery {
	query := (&ArmorClient{config: acq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(armorclass.Table, armorclass.FieldID, selector),
...
func (acq *ArmorClassQuery) WithArmor(opts ...func(*ArmorQuery)) *ArmorClassQuery {
	query := (&ArmorClient{config: acq.config}).Query()
...
func (acq *ArmorClassQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ArmorClass, error) {
	...
	if query := acq.withArmor; query != nil {
		if err := acq.loadArmor(ctx, query, nodes, nil,
			func(n *ArmorClass, e *Armor) { n.Edges.Armor = e }); err != nil {
			return nil, err
		}
	}
...
func (acq *ArmorClassQuery) loadArmor(ctx context.Context, query *ArmorQuery, nodes []*ArmorClass, init func(*ArmorClass), assign func(*ArmorClass, *Armor)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*ArmorClass)
	for i := range nodes {
		if nodes[i].armor_armor_class == nil {
			continue
		}
		fk := *nodes[i].armor_armor_class
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "armor_armor_class" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
