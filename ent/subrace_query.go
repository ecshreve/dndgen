...
type SubraceQuery struct {
	...
	withNamedLanguageOptions map[string]*LanguageChoiceQuery
...
func (sq *SubraceQuery) QueryLanguageOptions() *LanguageChoiceQuery {
	query := (&LanguageChoiceClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *SubraceQuery) Clone() *SubraceQuery {
	...
	return &SubraceQuery{
		...
		withLanguageOptions: sq.withLanguageOptions.Clone(),
...
func (sq *SubraceQuery) WithLanguageOptions(opts ...func(*LanguageChoiceQuery)) *SubraceQuery {
	query := (&LanguageChoiceClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withLanguageOptions = query
	return sq
}
...
func (sq *SubraceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Subrace, error) {
	...
	if query := sq.withLanguageOptions; query != nil {
		if err := sq.loadLanguageOptions(ctx, query, nodes,
			func(n *Subrace) { n.Edges.LanguageOptions = []*LanguageChoice{} },
			func(n *Subrace, e *LanguageChoice) { n.Edges.LanguageOptions = append(n.Edges.LanguageOptions, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range sq.withNamedLanguageOptions {
		if err := sq.loadLanguageOptions(ctx, query, nodes,
			func(n *Subrace) { n.appendNamedLanguageOptions(name) },
			func(n *Subrace, e *LanguageChoice) { n.appendNamedLanguageOptions(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (sq *SubraceQuery) loadAbilityBonuses(ctx context.Context, query *AbilityBonusQuery, nodes []*Subrace, init func(*Subrace), assign func(*Subrace, *AbilityBonus)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(subrace.AbilityBonusesTable)
		s.Join(joinT).On(s.C(abilitybonus.FieldID), joinT.C(subrace.AbilityBonusesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(subrace.AbilityBonusesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(subrace.AbilityBonusesPrimaryKey[0]))
...
func (sq *SubraceQuery) loadLanguageOptions(ctx context.Context, query *LanguageChoiceQuery, nodes []*Subrace, init func(*Subrace), assign func(*Subrace, *LanguageChoice)) error {
	...
	query.Where(predicate.LanguageChoice(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(subrace.LanguageOptionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.subrace_language_options
		if fk == nil {
			return fmt.Errorf(`foreign-key "subrace_language_options" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "subrace_language_options" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
// WithNamedLanguageOptions tells the query-builder to eager-load the nodes that are connected to the "language_options"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (sq *SubraceQuery) WithNamedLanguageOptions(name string, opts ...func(*LanguageChoiceQuery)) *SubraceQuery {
	query := (&LanguageChoiceClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if sq.withNamedLanguageOptions == nil {
		sq.withNamedLanguageOptions = make(map[string]*LanguageChoiceQuery)
	}
	sq.withNamedLanguageOptions[name] = query
	return sq
}
