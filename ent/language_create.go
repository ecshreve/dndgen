...
func (lc *LanguageCreate) AddTypicalSpeakerIDs(ids ...int) *LanguageCreate {
	lc.mutation.AddTypicalSpeakerIDs(ids...)
	return lc
}

// AddTypicalSpeakers adds the "typical_speakers" edges to the Race entity.
func (lc *LanguageCreate) AddTypicalSpeakers(r ...*Race) *LanguageCreate {
	ids := make([]int, len(r))
	for i := range r {
		ids[i] = r[i].ID
	}
	return lc.AddTypicalSpeakerIDs(ids...)
}
...
func (lc *LanguageCreate) createSpec() (*Language, *sqlgraph.CreateSpec) {
	...
	if nodes := lc.mutation.TypicalSpeakersIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   language.TypicalSpeakersTable,
			Columns: []string{language.TypicalSpeakersColumn},
			Bidi:    false,
