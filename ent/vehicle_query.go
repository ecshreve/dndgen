...
func (vq *VehicleQuery) Clone() *VehicleQuery {
	...
	return &VehicleQuery{
		...
		withEquipment: vq.withEquipment.Clone(),
...
func (vq *VehicleQuery) loadEquipment(ctx context.Context, query *EquipmentQuery, nodes []*Vehicle, init func(*Vehicle), assign func(*Vehicle, *Equipment)) error {
	...
	nids := make(map[int]map[*Vehicle]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(vehicle.EquipmentTable)
		s.Join(joinT).On(s.C(equipment.FieldID), joinT.C(vehicle.EquipmentPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(vehicle.EquipmentPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(vehicle.EquipmentPrimaryKey[1]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*Vehicle]struct{}{byID[outValue]: {}}
