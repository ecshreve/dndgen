...
func (tq *ToolQuery) Clone() *ToolQuery {
	...
	return &ToolQuery{
		...
		withEquipment: tq.withEquipment.Clone(),
...
func (tq *ToolQuery) loadEquipment(ctx context.Context, query *EquipmentQuery, nodes []*Tool, init func(*Tool), assign func(*Tool, *Equipment)) error {
	...
	for i := range nodes {
		if nodes[i].tool_equipment == nil {
			continue
		}
		fk := *nodes[i].tool_equipment
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "tool_equipment" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
