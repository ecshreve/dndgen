...
func (pcq *ProficiencyChoiceQuery) QueryOptions() *ProficiencyQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, false, proficiencychoice.OptionsTable, proficiencychoice.OptionsPrimaryKey...),
		)
...
func (pcq *ProficiencyChoiceQuery) QueryRace() *RaceQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, true, proficiencychoice.RaceTable, proficiencychoice.RaceColumn),
		)
		fromU = sqlgraph.SetNeighbors(pcq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (pcq *ProficiencyChoiceQuery) Clone() *ProficiencyChoiceQuery {
	...
	return &ProficiencyChoiceQuery{
		...
		withOptions: pcq.withOptions.Clone(),
...
func (pcq *ProficiencyChoiceQuery) WithOptions(opts ...func(*ProficiencyQuery)) *ProficiencyChoiceQuery {
	query := (&ProficiencyClient{config: pcq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pcq.withOptions = query
	return pcq
}
...
//		GroupBy(proficiencychoice.FieldDesc).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.ProficiencyChoice.Query().
//		Select(proficiencychoice.FieldDesc).
//		Scan(ctx, &v)
...
func (pcq *ProficiencyChoiceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ProficiencyChoice, error) {
	...
	if query := pcq.withOptions; query != nil {
		if err := pcq.loadOptions(ctx, query, nodes,
			func(n *ProficiencyChoice) { n.Edges.Options = []*Proficiency{} },
			func(n *ProficiencyChoice, e *Proficiency) { n.Edges.Options = append(n.Edges.Options, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range pcq.withNamedOptions {
		if err := pcq.loadOptions(ctx, query, nodes,
			func(n *ProficiencyChoice) { n.appendNamedOptions(name) },
			func(n *ProficiencyChoice, e *Proficiency) { n.appendNamedOptions(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (pcq *ProficiencyChoiceQuery) loadOptions(ctx context.Context, query *ProficiencyQuery, nodes []*ProficiencyChoice, init func(*ProficiencyChoice), assign func(*ProficiencyChoice, *Proficiency)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(proficiencychoice.OptionsTable)
		s.Join(joinT).On(s.C(proficiency.FieldID), joinT.C(proficiencychoice.OptionsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(proficiencychoice.OptionsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(proficiencychoice.OptionsPrimaryKey[0]))
...
func (pcq *ProficiencyChoiceQuery) loadRace(ctx context.Context, query *RaceQuery, nodes []*ProficiencyChoice, init func(*ProficiencyChoice), assign func(*ProficiencyChoice, *Race)) error {
	...
	query.Where(predicate.Race(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(proficiencychoice.RaceColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.race_starting_proficiency_option
		if fk == nil {
			return fmt.Errorf(`foreign-key "race_starting_proficiency_option" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "race_starting_proficiency_option" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (pcq *ProficiencyChoiceQuery) WithNamedOptions(name string, opts ...func(*ProficiencyQuery)) *ProficiencyChoiceQuery {
	query := (&ProficiencyClient{config: pcq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if pcq.withNamedOptions == nil {
		pcq.withNamedOptions = make(map[string]*ProficiencyQuery)
	}
	pcq.withNamedOptions[name] = query
	return pcq
}
