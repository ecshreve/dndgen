...
func (ecc *EquipmentCostCreate) check() error {
	...
	if _, ok := ecc.mutation.GpValue(); !ok {
		return &ValidationError{Name: "gp_value", err: errors.New(`ent: missing required field "EquipmentCost.gp_value"`)}
	}
	if _, ok := ecc.mutation.EquipmentID(); !ok {
...
func (ecc *EquipmentCostCreate) createSpec() (*EquipmentCost, *sqlgraph.CreateSpec) {
	...
	if value, ok := ecc.mutation.GpValue(); ok {
		_spec.SetField(equipmentcost.FieldGpValue, field.TypeFloat64, value)
		_node.GpValue = value
	}
